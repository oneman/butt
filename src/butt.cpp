...
#ifndef BUILD_CLIENT
    printf(
        "Usage: butt [-h | -v | -s [name] | -u <song name> | -M <streaming signal threshold> | -m <streaming silence threshold> | -O <recording signal threshold> | -o <recording silence threshold> | -Sdrtqn | -c <config_path>] [-A | -U | -x] [-a <addr>] [-p <port>]\n");
#else
    printf(
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, ":vhc:AUxs:drtnqu:a:p:SM:m:O:o:")) != -1) {
        switch (opt) {
#ifndef BUILD_CLIENT
        case 'A':
